...
/// [CurrentStateViewModel] after all [CurrentProperty] values have been initialized.
///
/// Generic helper implementations can be exposed from
/// [CurrentStateViewModel.currentBindings].
abstract class CurrentViewModelBinding {
  /// Attaches this helper to its owning [CurrentStateViewModel].
  ///
  /// This is called automatically from the [CurrentStateViewModel] constructor after
  /// all [CurrentStateViewModel.currentProps] have been associated with the view model.
...
///
///This is where any logic that manipulates your model should be. In Flutter the CurrentWidget, or
///in Jaspr the CurrentComponent, and it's accompanying state always has access to it's view model via the viewModel property.
///
///Update events are automatically emitted whenever the value of an [CurrentProperty] is changed.
///The state the ViewModel is bound to will update itself each time an [CurrentProperty] value
///is changed and call the states build function, updating the UI.
abstract class CurrentStateViewModel {
  final StreamController<CurrentStateChanged> _stateController = StreamController.broadcast();
  ...
  /// Notifies components attached to this view model that the state has changed and they should update themselves.
  ///
  void notifyListeners();

  ///Creates associates the view model with a specific state via the states hash code.
  ///
  ///This method is called automatically by the state when the view model is assigned to a state.
  ///
  ///**This method should not be called manually.**
  void assignTo(int widgetHash) {
    if (assignedToState && assignedTo != widgetHash) {
      throw CurrentViewModelAlreadyAssignedException(StackTrace.current, runtimeType);
    }
    _assignedTo = widgetHash;
  }

  /// Releases the current state assignment when it matches the
  ...
  StreamSubscription<ErrorEvent> addAnyErrorEventListener(
    ...
    return addOnErrorEventListener<ErrorEvent>(onError, onInternalError: onInternalError);
  }
  ...
  void setBusyStatus({required bool isBusy, dynamic busyTaskKey}) {
    if (_busy != isBusy || busyTaskKey != null) {
      ...
      notifyChange(BusyStatusChanged(isBusy: _busy, busyTaskKey: busyTaskKey));
    }
  }
  ...
  ///
  /// **NOTE** Any change to the busy status will trigger a UI update in the bound state, regardless of whether you use this function to subscribe to busy status changes or not.
  ...
  StreamSubscription addBusyStatusChangedListener(
    ...
    return addStateChangedListener<BusyStatusChanged>(
      onChanged,
      filter: (event) => busyTaskKey == null || event.busyTaskKey == busyTaskKey,
    );
  }
...
///[CurrentStateViewModel.addStateChangedListener] function will receive these types of events
class CurrentStateChanged<T> {
  ...
  static CurrentStateChanged addedToList<V>(V newValue, {String? propertyName, int? sourceHashCode}) =>
      CurrentStateChanged(
        newValue,
        null,
        propertyName: propertyName,
  ...
  static CurrentStateChanged addedAllToList<V>(
    Iterable<V> newValues, {
    Iterable<V>? previousValue,
    String? propertyName,
    int? sourceHashCode,
  }) => CurrentStateChanged(
    newValues,
  ...
  static CurrentStateChanged insertIntoList<V>(
    int index,
    V value, {
    String? propertyName,
    int? sourceHashCode,
  ...
  static CurrentStateChanged insertAllIntoList<V>(
    int index,
    Iterable<V> values, {
    Iterable<V>? previousValue,
    String? propertyName,
  ...
  static CurrentStateChanged removedFromList<V>(
    V removedValue, {
    String? propertyName,
    int? sourceHashCode,
  }) => CurrentStateChanged(
  ...
  static CurrentStateChanged addedMapToMap<K, V>(
    Map<K, V> addedMap, {
    Map<K, V>? previousValue,
    String? propertyName,
    int? sourceHashCode,
  }) {
  ...
  static CurrentStateChanged addedToMap<K, V>(
    K key,
    V newValue, {
    String? propertyName,
    int? sourceHashCode,
  ...
  static CurrentStateChanged addedEntriesToMap<K, V>(
    ...
    Iterable<MapEntry<K, V>>? previousValue,
    String? propertyName,
    int? sourceHashCode,
  }) {
    return CurrentStateChanged(
  ...
  static CurrentStateChanged<V> removedFromMap<K, V>(
    K key,
    V? removedValue, {
    String? propertyName,
    int? sourceHashCode,
...
///
/// Any event handlers registered with the [CurrentStateViewModel.addBusyStatusChangedListener] function will receive these types of events.
...
///
///Any event handlers registered with the [CurrentStateViewModel.addStateChangedListener] or [CurrentStateViewModel.addAnyStateChangedListener] functions will
