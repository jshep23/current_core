...
class _CollectionViewModel extends CurrentStateViewModel {
...
class _CollectionResetViewModel extends CurrentStateViewModel {
...
void main() {
  ...
  test('resetAll - change property values - All values equal their original value', () {
  ...
  test('isDirty - change property values and update original values - isDirty is false', () {
    viewModel.name('Steve');
    viewModel.age(100);
  ...
  test('resetAll - collection properties do not alias original values', () {
    ...
    expect(collectionResetViewModel.data.originalValue, equals({'planet': 'Earth'}));
  ...
  test('setMultiple - mixed scalar changes - emits only changed events', () async {
    ...
    expect(receivedEvents.single.propertyName, equals(viewModel.age.propertyName));
  ...
  test('setMultiple - collection values with equal contents - emits no events', () async {
    var eventCount = 0;

    final subscription = collectionResetViewModel.addAnyStateChangedListener((_) {
      eventCount++;
    });
